...
# k6 CSVs can be large, so we only load the columns we actually need
df = pd.read_csv(CSV_FILE, usecols=['metric_name', 'timestamp', 'metric_value'])

# Convert k6 Unix timestamp (seconds) to actual datetime objects for plotting
...
# Resample the data into 1-second buckets and count the requests in each bucket
# Because each row is 1 request, .count() gives us RPS
rps = df_reqs['metric_value'].resample('1s').count()

# 4. Filter and Process Latency (p95 and Average)
...
# Define a custom function for the 95th percentile
def p95(x):
    return np.percentile(x, 95) if len(x) > 0 else np.nan

 # Resample latency into 1-second buckets to match RPS
latency = df_duration['metric_value'].resample('1s').agg(['mean', p95])
# Ensure latency is a DataFrame before assigning columns
...
plt.style.use('default') # Use a clean, standard style
fig, ax1 = plt.subplots(figsize=(10, 6), dpi=300) # High DPI for paper publishing

# Plot Latency on the primary Y-axis (left)
color_p95 = 'tab:red'
color_avg = 'tab:orange'
ax1.set_xlabel('Test Duration (Time)')
...
# Create a secondary Y-axis for Throughput (right)
ax2 = ax1.twinx()  
color_rps = 'tab:blue'
...
# Fill the area under the RPS curve for better visual impact
